...
#    else:
  ...
  def trainSplit(self, trainFile):
    ...
    self.train_df = pd.read_csv(trainFile,usecols=['text', 'is_humor'])
    for idx, row in self.train_df.iterrows():
        if not np.isnan(row['is_humor']):
            example = self.Example()
            example.words = self.segmentWords(row['text'])
            example.klass = str(int(row['is_humor']))
...
#      split.train.append(example)
  ...
  def crossValidationSplits(self, train_file):
    ...
    example_list = []
    self.train_df = pd.read_csv(train_file, usecols=['text','is_humor'])

    for idx, row in self.train_df.iterrows():
        example = self.Example()
        example.words = self.segmentWords(row['text'])
        example.klass = str(row['is_humor'])
